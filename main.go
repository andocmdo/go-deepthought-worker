...
func main() {
	...
	ipPort := flag.String("master", "127.0.0.1:8080", "IP address and port of the API server (master node)")
	...
	master = server{URLroot: "http://" + *ipPort, URLjobs: "http://" + *ipPort + *api + "jobs", URLworkers: "http://" + *ipPort + *api + "workers"}
